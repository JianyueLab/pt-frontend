<template>
  ...
  <div v-else-if="data" class="space-y-6">
    ...
    <div class="pb-5 border-b border-default">
      ...
      <div
        class="flex flex-wrap items-center gap-x-5 gap-y-1 mt-2 text-sm text-muted"
      >
        <span
          >AS Number
          <strong class="text-highlighted">{{ data.asn }}</strong></span
        >
        <a
          v-if="data.website"
          :href="data.website"
    ...
    <UTabs :key="tabs.map(t => t.value).join()" :items="tabs" default-value="overview" color="neutral">
      <template #content="{ item }">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-5">
            ...
            <div>
              ...
              <div class="font-medium text-base">
                {{ data.v4_count }} IPv4, {{ data.v6_count }} IPv6
              </div>
            </div>
            ...
            <div v-if="data.sponsor_org">
              ...
              <div class="font-medium text-base">
                {{ data.sponsor_org.name }}
              </div>
              <div class="text-dimmed text-sm">
                {{ data.sponsor_org.handle }}
              </div>
            </div>
          </div>
            ...
            <div class="rounded-xl border border-default overflow-hidden">
              <table class="w-full text-sm">
                <thead class="bg-elevated text-left">
                  <tr>
                    <th
                      class="px-4 py-2.5 text-xs font-medium text-dimmed uppercase tracking-wide"
                    >
                      ASN
                    </th>
                    <th
                ...
                <tbody>
                  <tr
                    v-for="a in data.relationships.upstream"
                    :key="a"
          ...
          <div v-if="data.relationships?.peers.length">
            ...
            <div class="rounded-xl border border-default overflow-hidden">
              <table class="w-full text-sm">
                ...
                <tbody>
                  <tr
                    v-for="a in data.relationships.peers"
                    :key="a"
                    class="border-t border-default hover:bg-elevated/50 transition-colors"
                  >
                    <td class="px-4 py-3 font-mono">
          ...
          <div v-if="data.relationships?.downstream.length">
            ...
            <div class="rounded-xl border border-default overflow-hidden">
              <table class="w-full text-sm">
                ...
                <tbody>
                  <tr
                    v-for="a in data.relationships.downstream"
                    :key="a"
                    class="border-t border-default hover:bg-elevated/50 transition-colors"
                  >
                    <td class="px-4 py-3 font-mono">
        ...
        <div v-else-if="item.value === 'prefixes'" class="pt-5 space-y-5">
          <div
            class="flex flex-wrap justify-between gap-6 pb-5 border-b border-default"
          >
            <div>
            ...
            <div class="text-right">
              ...
              <div class="font-semibold text-base">
                {{ data.v4_size.toLocaleString() }} /24's of IPv4
              </div>
              <div class="font-semibold text-base">
                {{ data.v6_size.toLocaleString() }} /48's of IPv6
              </div>
            </div>
          </div>
...
watchEffect(() => {
  if (data.value) {
    useHead({
      title: `AS${data.value.asn} — ${displayName.value} | Player Tools`,
    });
  }
});
...
const asnInfo = (n: number) => {
  ...
  return entry
    ? { known: true, ...entry }
    : { known: false, name: "", v4: false, v6: false };
};
