<template>
  <div class="space-y-4">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
      ...
      <StatCard v-if="d.country" label="Country" :value="d.country" icon="i-lucide-map-pin" />
    </div>

    <div v-if="d.org || d.sponsor_org" class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
      <div v-if="d.org" class="p-3 border border-default flex items-start gap-2">
        <UIcon name="i-lucide-building-2" class="text-dimmed mt-0.5 shrink-0" />
        <div>
          <div class="text-dimmed text-xs mb-1">Org</div>
          <div class="font-medium">{{ d.org.name }}</div>
          <div class="text-dimmed">{{ d.org.handle }}</div>
        </div>
      </div>
      <div v-if="d.sponsor_org" class="p-3 border border-default flex items-start gap-2">
        <UIcon name="i-lucide-handshake" class="text-dimmed mt-0.5 shrink-0" />
        <div>
          <div class="text-dimmed text-xs mb-1">Sponsor</div>
    ...
    <a v-if="d.website" :href="d.website" target="_blank" rel="noopener" class="text-sm text-primary hover:underline inline-flex items-center gap-1">
      <UIcon name="i-lucide-external-link" class="text-xs" />
      {{ d.website }}
    </a>

    <pre v-if="d.comments" class="text-sm text-muted p-3 border border-default whitespace-pre-wrap font-sans">{{ d.comments }}</pre>
  </div>
</template>
